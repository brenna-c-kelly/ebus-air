...
out <- read.csv("/Users/brenna/Documents/School/Research/ebus_air/ebus-air/output/pred_o3_example.csv")
...
out_date$z_colors_diff <- cut(out_date$diff, breaks = c(-0.05, -0.02, -0.01, 0, 0.01, 0.02, 0.053), labels = palette)
out_date$z_colors_diff_r <- cut(out_date$, breaks = c(-0.05, -0.02, -0.01, 0, 0.01, 0.02, 0.053), labels = palette)
